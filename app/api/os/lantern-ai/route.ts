...
- ALWAYS add a blank line before and after each section
- NEVER crowd information together - readability is paramount
- Use consistent indentation: 2 spaces for sections, 4 spaces for tasks
- Each task gets its own block with clear separation
- Use horizontal rules to separate team members
- Keep metadata on same line as task when possible for scannability

Example response for "what does the team need to do this week":


**BLOCKERS (2)** - Resolve First

    "API rate limit issue"
    Project: HireWire  |  Owner: Rory  |  Priority: critical

    "Invoice sync failing"
    Project: Paradise  |  Owner: Keymon  |  Priority: high
...
- Add blank lines between each task for clarity`
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (weeklyByMember.length || unassignedThisWeek.length) {
      // Header section with clear borders
      lines.push('')
      lines.push('')
      lines.push('╔══════════════════════════════════════════════════════════╗')
      ...
      // Week totals in a clean box
      lines.push('┌─────────────────────────────────────┐')
      lines.push('│  WEEK TOTALS                        │')
      lines.push('├─────────────────────────────────────┤')
      lines.push(`│  Tasks Due:     ${String(weekTotalTasks).padEnd(18)}  │`)
      lines.push(`│  Overdue:       ${String(weekTotalOverdue).padEnd(18)}  │`)
      lines.push(`│  Blocked:       ${String(weekTotalBlocked).padEnd(18)}  │`)
      lines.push(`│  Critical:      ${String(weekTotalCritical).padEnd(18)}  │`)
      lines.push('└─────────────────────────────────────┘')
      lines.push('')
      lines.push('')
      ...
      if (allBlockedThisWeek.length) {
        lines.push('┌─────────────────────────────────────────────────────────┐')
        lines.push(`│  EXECUTION BLOCKERS (${allBlockedThisWeek.length})                                   │`)
        lines.push('│  These need resolution before progress can continue     │')
        lines.push('└─────────────────────────────────────────────────────────┘')
        lines.push('')
        
        allBlockedThisWeek.forEach((t, idx) => {
          const tenant = tenantMap.get(t.tenant_id)?.name ?? t.tenant_id
          const owner = t.owner_user_id ? userMap.get(t.owner_user_id)?.name ?? 'unassigned' : 'UNASSIGNED'
          ...
          if (t.blocker_reason) {
            lines.push(`       Reason:    ${t.blocker_reason}`)
          }
          lines.push('')
          lines.push('')
        })
      ...
      weeklyByMember.forEach(member => {
        ...
        // Stats line
        const stats = [`${member.totalThisWeek} tasks this week`]
        if (member.blocked) stats.push(`${member.blocked} blocked`)
        if (member.overdue) stats.push(`${member.overdue} overdue`)
        if (member.inProgress) stats.push(`${member.inProgress} in progress`)
      ...
      if (unassignedThisWeek.length || unassignedOverdue.length) {
        lines.push('')
        lines.push('═══════════════════════════════════════════════════════════')
        lines.push('  UNASSIGNED TASKS - NEED OWNER')
        lines.push('═══════════════════════════════════════════════════════════')
        lines.push('')
        lines.push(`    ${unassignedThisWeek.length + unassignedOverdue.length} tasks without an assigned owner`)
        lines.push('')
      ...
      lines.push('')
      lines.push('╔══════════════════════════════════════════════════════════╗')
      lines.push('║  END OF WEEKLY TASK BREAKDOWN                            ║')
      lines.push('╚══════════════════════════════════════════════════════════╝')
      lines.push('')
    }
