...
export function CommsClient({
  ...
  // When active tenant changes, switch to that tenant's first channel
  useEffect(() => {
  ...
  // Fetch messages + realtime when channel changes
  useEffect(() => {
        ...
        <div style={{ padding: '8px 20px 16px', borderTop: '1px solid rgba(255,255,255,0.07)', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', background: '#18181B', border: '1px solid rgba(255,255,255,0.1)', borderRadius: replyTo ? '0 0 8px 8px' : 8, padding: '8px 10px 8px 14px' }}>
            <textarea
              ...
              style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: '#FAFAFA', fontSize: 13, fontFamily: 'inherit', resize: 'none', lineHeight: 1.5, maxHeight: 120, overflow: 'hidden' }}
            />
            <button
          ...
          {members.map(m => (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 14px' }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <p style={{ fontSize: 11, fontWeight: 600, color: m.id === authUser.id ? '#FAFAFA' : '#A1A1AA', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', margin: 0 }}>
                  {m.name}{m.id === authUser.id ? ' (you)' : ''}
                </p>
                {m.role && <p style={{ fontSize: 9, color: '#3F3F46', textTransform: 'uppercase', letterSpacing: 0.5, margin: 0 }}>{m.role}</p>}
              </div>
            </div>
          ))}
        </div>
